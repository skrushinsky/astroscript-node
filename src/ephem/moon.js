...
************************************************************************************/
const { radians, frac, polynome, reduceDeg } = require ('../mathutils.js');
...
module.exports = {
    ...
    truePosition(dj) {
        ...
        let a  = radians(51.2 + 20.2 * t);
        let sa = sin(a);
        let sn = sin(radians(n));
        let b = 346.56 + (132.87 - 9.1731E-3 * t) * t;
        let sb = 3.964E-3 * sin(radians(b));
        let c = radians(n + 275.05 - 2.3 * t);
        let sc = sin(c);
        ...
        const e2 = e * e;
        [ ms, n, de, f, md ] = [ ms, n, de, f, md ].map( x => radians(x) );
        const de2 = de + de;
        const de3 = de2 + de;
        const de4 = de2 + de2;
        const md2 = md + md;
        ...
        let lambda = radians(reduceDeg(ld + l));


        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // ecliptic latitude
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        const g = 5.128189 * sin(f)
               + .280606 * sin(md + f)
               + .277693 * sin(md - f)
               + .173238 * sin(de2 - f)
               + .055413 * sin(de2 + f - md)
               ...
               - .000283 * sin(md + f3);
        let w1 = .0004664 * cos(n);
        let w2 = .0000754 * cos(c);
        let beta = radians(g * (1 - w1 - w2));

        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // horizontal parallax
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        let hp = .950724 + .051818 * cos(md)
               + .009531 * cos(de2 - md)
               + .007843 * cos(de2)
               + .002824 * cos(md2)
               + .000857 * cos(de2 + md)
        ...
        return [ lambda, beta, delta, hp, dm ];
    },

    // Mean Lunar Node
    // meanNode: function(t) {
    //    return radians(reduceDeg(2.591833e2 + 2.078e-3 * t * t - frac360(5.372616667 * t)));
    //},

    node: function(djd, trueNode=true) {
        ...
        let nd;
        if (trueNode) {
            const [d, m, f] = ['D', 'M', 'L'].map( k =>
                radians(reduceDeg(polynome.apply(this, [t].concat(_MOO_ORBIT[k])))) );
            const ms = radians(reduceDeg(polynome.apply(this, [t].concat(_SUN_ORBIT['M']))));
